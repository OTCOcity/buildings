                        ...
                        <table align="left" border="0" cellpadding="0" cellspacing="0" width="100%" style="min-width:100%;border-collapse:collapse">
                            <tbody><tr>
                                <td valign="top" style="padding:9px 18px;font-size:14px;word-break:break-word;color:#202020;font-family:Helvetica;line-height:150%;text-align:left">
                                    ...
                                    <p><b>Стоимость закза:</b> <?=$order->sum_order?> Р</p>
                                    <p><b>Стоимость доставки:</b> индивидуальный расчет</p>
                                    <p><b>Общая стоимость:</b> <?=$order->sum_itog?> Р</p>
                                    <br>
                                    <h3>Ваши данные</h3>
                                    <p><b>Вы:</b> <?=$order->typestr?></p>
                                    <p><b>Имя:</b> <?=$order->client_name?></p>
                                    <p><b>Телефон:</b> <?=$order->client_phone?></p>
                                    <p><b>Email:</b> <?=$order->client_email?></p>
                                    <?php if ($order->type == 2): ?>
                                        <p><b>Компания:</b> <?=$order->client_company;?></p>
                                    <?php endif; ?>
                                    <br>
                                    <h3>Доставка и оплата</h3>
                                    <p><b>Тип доставки:</b> <?=$order->delivtypestr?></p>
                                    <p><b>Тип оплаты:</b> <?=$order->paytypestr?></p>
                                    <?php if  ($order->deliv_type == 2): ?>
                                        <p><b>Адрес доставки:</b> <?=$order->deliv_city?> <?=$order->deliv_adres?></p>
                                        <p><b>Время доставки:</b> <?=$order->deliv_date?></p>
                                        <p><b>Время доставки:</b> <?=$order->deliv_time?></p>
                                    <?php endif; ?>
                                    <br>
                                    ...
                                    <table>
                                        ...
                                        <?php foreach ($order->items as $key => $item):?>
                                            <tr>
                                                ...
                                                <td style="font-size: 12px; text-align: left;padding: 0 10px;"><?=$item->name;?> (<?=$item->subname;?>)</td>
                                                <td style="font-size: 12px; text-align: center;padding: 0 10px;"><?=$item->price;?></td>
                                                <td style="font-size: 12px; text-align: center;padding: 0 10px;"><?=$item->count;?></td>
                                                <td style="font-size: 12px; text-align: center;padding: 0 10px;"><?=$item->sum;?></td>
                                            </tr>
                                        <?php endforeach;?>
                                    </table>
                                </td>
